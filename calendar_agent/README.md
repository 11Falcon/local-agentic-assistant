...
built for Gmail — only the scope (`https://www.googleapis.com/auth/calendar`) and the
...
**Before the tasks:** nothing new — the auth is the Gmail pattern with two strings
...
Verified by [`tests/test_calendar_agent.py`](tests/test_calendar_agent.py) — `python checker.py calendar_agent`
