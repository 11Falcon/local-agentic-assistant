...
**An agent = model + system prompt + tools + loop + state.** In tool calling you wrote the
...
Calendar and notes agents and the orchestrator will import. This
...
- *Observability.* Real agent frameworks record a **trace** — every user input, tool
  ...
  You'll add one; in the final assistant it becomes your debugging lifeline.
...
your own tool-calling code plus this README's specs.
...
Verified by [`tests/test_agent_core.py`](tests/test_agent_core.py) — `python checker.py agent_core`
